...
def gerar_dp_opr_daily(rows: list[dict]) -> str:
    ...
    campos = ["dia","loja","uid","nome","pedidos","prontos","em_producao",
              "d30","d120","acima120","f_sem_log_vanilla","f_sem_log_gap","mediana"]
    linhas = []
    for r in rows:
        vals = []
        for c in campos:
...
def main():
    ...
    lp_rows = []
    try:
        lp_rows = mb_query(token, SQL_MIX_PRODUTOS.format(ini_dp=ini_dp, fim=fim), limit=300, retries=1)
    ...
    html = injetar_no_html(html, dp_raw_js, dp_opr_daily_js, an_js, an_dist_js, hoje, lp_todo=lp_js)
